...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'tube-dist/video-sikh-social/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'tube-dist/video-sikh-social/browser'), 'index.html')
    )
    .toString();
      ...
      async (err, html) => {
        ...
        if (params.indexOf('channel/') > -1) {
          ...
          const talent = {
            ...
            image: post?.thumbfilename || post?.metaimage || post?.imageUrl || 'https://tube.sikh.social/assets/Healing-Tube-Logo.png',
          };
          seo.title = talent.name;
          seo.description = strip_html_tags(talent.description);
          seo.image = talent.image;
